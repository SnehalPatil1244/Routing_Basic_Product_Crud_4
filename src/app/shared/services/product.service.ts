...
export class ProductService {
  ...
  catreproduct(product: IProduct): Observable<IRes<IProduct>> {
    this.productsArr.push(product)
    return of({
      msg: `The New Product With ID ${product.pid} Is Added Successfully !!`,
      data: product
    })
  }
  ...
  updatedroduct(updtedobj: IProduct): Observable<IRes<IProduct>> {
    let getindex = this.productsArr.findIndex(p => p.pid === updtedobj.pid)
    this.productsArr[getindex] = updtedobj
    return of({
      msg: `the Product With Id ${updtedobj.pid} Is Updated Successfully !!!`,
      data: updtedobj
    })
  }
}
